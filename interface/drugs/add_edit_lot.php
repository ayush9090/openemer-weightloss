...
if (!empty($_POST['form_save'])) {
    ...
    if ($form_source_lot) {
        ...
        if (empty($form_lot_number)) {
            $form_lot_number = is_array($srow) ? $srow['lot_number'] : '';
        }
        if (empty($form_expiration)) {
             $form_expiration = is_array($srow) ? $srow['expiration'] : '';
        }
        if (empty($form_manufacturer)) {
             $form_manufacturer = is_array($srow) ? $srow['manufacturer'] : '';
        }
        if (empty($form_vendor_id)) {
             $form_vendor_id = is_array($srow) ? $srow['vendor_id'] : '';
        }
    ...
    if (!$info_msg) {
        ...
        if (!$lot_id && $form_lot_number && ($form_trans_type == 2 || $form_trans_type == 4)) {
            ...
            if (!empty($erow['inventory_id'])) {
                ...
                if (empty($form_expiration)) {
                    $form_expiration   = is_array($erow) ? $erow['expiration'] : '';
                }
                if (empty($form_manufacturer)) {
                    $form_manufacturer = is_array($erow) ? $erow['manufacturer'] : '';
                }
                if (empty($form_vendor_id)) {
                    $form_vendor_id    = is_array($erow) ? $erow['vendor_id'] : '';
                }
            }
        }
            ...
            if ($_POST['form_save']) {
                ...
                if (($row['on_hand'] + $form_quantity) < 0) {
                    ...
                    sqlStatement(
                        ...
                        array(
                            $drug_id,
                            trim($_POST['form_lot_number']),
                            trim($_POST['form_expiration']),
                            trim($_POST['form_manufacturer']),
                            trim($_POST['form_on_hand']),
                            trim($_POST['form_warehouse_id']),
                            trim($_POST['form_vendor_id']),
                            trim($_POST['form_clinic_id']),
                            $lot_id
                        )
                    );
                }
            ...
            if ($form_quantity < 0) {
                ...
                if ($crow['count']) {
                    ...
                    $lot_id = sqlInsert(
                        ...
                        "warehouse_id, vendor_id, clinic_id " .
                        ") VALUES ( ?, ?, ?, ?, ?, ?, ?, ? )",
                        array(
                            $drug_id,
                            trim($_POST['form_lot_number']),
...
?>
 ...
 <tr id='row_lot_number'>
  ...
  <td>
   <input class="form-control w-100" type='text' size='40' name='form_lot_number' maxlength='40' value='<?php echo attr(is_array($row) ? $row['lot_number'] : '') ?>' />
  </td>
 </tr>
 ...
 <tr id='row_manufacturer'>
  ...
  <td>
   <input class="form-control w-100" type='text' size='40' name='form_manufacturer' maxlength='250' value='<?php echo attr(is_array($row) ? $row['manufacturer'] : '') ?>' />
  </td>
 </tr>
 ...
 <tr id='row_expiration'>
  ...
  <td>
   <input type='text' class='datepicker form-control w-50' size='10' name='form_expiration' id='form_expiration'
    value='<?php echo attr(is_array($row) ? $row['expiration'] : '') ?>'
...
?>
 ...
 <tr id="row_vendor" style="display:none">
...
?>
 ...
 <tr id='row_on_hand'>
  ...
  <td>
    <span><?php echo text(is_array($row) && isset($row['on_hand']) ? ($row['on_hand'] + 0) : ''); ?></span>
  </td>
 </tr>
 ...
 <div class='form-group'>
  ...
  <select name='form_clinic_id' class='form-control'>
<?php
...
while ($clinic = sqlFetchArray($clinics)) {
    ...
    if ($clinic['id'] == $row['clinic_id']) echo " selected";
