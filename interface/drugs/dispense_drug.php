...
// Get the clinic_id from the current user's facility
$clinic_id = $_SESSION['authUserClinicId'];

// Modify the inventory check to include clinic_id
$inventory = sqlQuery(
    "SELECT di.* FROM drug_inventory di " .
    "WHERE di.drug_id = ? AND di.clinic_id = ? AND di.on_hand > 0 " .
    "ORDER BY di.expiration ASC LIMIT 1",
    array($drug_id, $clinic_id)
);

if (!$inventory) {
    echo "<div class='alert alert-warning'>" . xlt('No inventory available for this clinic') . "</div>";
    exit;
}

// When dispensing, update the clinic-specific inventory
sqlStatement(
    "UPDATE drug_inventory SET " .
    "on_hand = on_hand - ? " .
    "WHERE inventory_id = ? AND clinic_id = ?",
    array($quantity, $inventory['inventory_id'], $clinic_id)
);

// Add clinic_id to the drug_sales record
sqlInsert(
    "INSERT INTO drug_sales ( " .
    "drug_id, inventory_id, prescription_id, pid, encounter, " .
    "user, sale_date, quantity, fee, clinic_id " .
    ") VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )",
    array(
        $drug_id,
