...
function toggleDrugFields() {
    ...
    var medicine = form.form_active;
    var product = form.form_consumable;
    ...
    function updateFieldVisibility() {
        var isMedicine = medicine.checked;
        var isProduct = product.checked;
        // Always show name
        document.querySelector('.form-group input[name="form_name"]').closest('.form-group').style.display = '';
        // Show/hide On Order
        var onOrderRow = document.querySelector('.on-order-row');
        if (onOrderRow) {
            onOrderRow.style.display = (isMedicine || isProduct) ? '' : 'none';
        }
        // Show/hide all other fields
        var allGroups = document.querySelectorAll('.form-group, .form-group.mt-3, .drugsonly, .ippfonly');
        allGroups.forEach(function(div) {
            // Skip name and on order
            if (
                div.querySelector('input[name="form_name"]') ||
                div.classList.contains('on-order-row')
            ) {
                return;
            }
            // Always show the attributes (checkbox) row
            var attrRow = document.querySelector('input[name="form_active"]').closest('.form-group');
            if (div === attrRow) {
                div.style.display = '';
                return;
            }
            // Show all fields for Medicine, only name/on order/attributes for Product
            if (isMedicine) {
                div.style.display = '';
            } else if (isProduct) {
...
window.addEventListener('DOMContentLoaded', function() {
    toggleDrugFields();
    // Hide all options in Route except By Mouth and Subcutaneous
    var routeSelect = document.getElementById('form_route');
    if (routeSelect) {
        Array.from(routeSelect.options).forEach(function(opt) {
            if (opt.text !== 'By Mouth' && opt.text !== 'Subcutaneous' && opt.value !== '0') {
...
<form method='post' name='theform' action='add_edit_drug.php?drug=<?php echo attr_url($drug_id); ?>'
 onsubmit='return validate(this);'>
    ...
    <div class="form-group mt-3 on-order-row">
