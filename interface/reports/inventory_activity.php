...
// Compute WHERE condition for filtering on facility and warehouse.
$fwcond = '';
if ($facility_id) {
    $fwcond .= " AND (lo.option_value = " . intval($facility_id) . " OR lo.option_value IS NULL)";
}
if ($form_warehouse) {
    $fwcond .= " AND di.warehouse_id = " . sqlEscape($form_warehouse);
}
...
function getEndInventory($product_id = 0, $warehouse_id = '~')
{
  ...
  // report end date (which is effectively a type of transaction).
    ...
    "$prodcond $whidcond $fwcond", array($form_to_date));
